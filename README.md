...
- Развертывание проекта на production-сервере осуществляется согласно docker-compose.production.yml. Инструкция предназначена для сервера с архитектурой ARM64v8.
- Перед развертыванием docker-compose.production.yml необходимо:
    - собрать образы backend, tg_bot, nginx с учетом архитектуры сервера (внести коррективы в Dockerfile)
